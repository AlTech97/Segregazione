...
//LOGICA PER LA VERIFICA DELLA SODDISFAZIONE DEGLI AGENTI E DEI LORO SPOSTAMENTI
	...
	//LOGICA DI CONTROLLO DEI CONFLITTI
		...
		for (i=0,j=0; j < COLONNE; j++) {
			...
			else if (sottomatrice[i][j] != 0) {	//agente di tipo 1 o 2, controlla la generazione dei conflitti con i vicini
				//raccolgo a fattor comune per evitare ripetizioni
				if (sottomatrice[i + 1][j] != 0) {
					numVicini++;
					if (sottomatrice[i][j] != sottomatrice[i + 1][j])
						conflitti++;
				}

				if (j == 0) {	//posizione 0,0 della sottomatrice
					if (sottomatrice[i][j + 1] != 0) {
						numVicini++;
						if (sottomatrice[i][j] != sottomatrice[i][j + 1])
							conflitti++;
					}
					if (sottomatrice[i + 1][j + 1] != 0) {
						numVicini++;
						if (sottomatrice[i][j] != sottomatrice[i + 1][j + 1])
							conflitti++;
					}
					if (rank != 0) {	//il rank 0 non legge la riga precedente
						if (rigaPred[j + 1] != 0) {
							numVicini++;
							if (sottomatrice[i][j] != rigaPred[j + 1])
								conflitti++;
						}
						if (rigaPred[j] != 0) {
							numVicini++;
							if (sottomatrice[i][j] != rigaPred[j])
								conflitti++;
						}
					}
					if (righe == 1 && (rank != numtasks-1)) {	//se il processo ha la sottomatrice con una sola riga deve confrontarla anche con la prima riga del successore
						if (rigaSucc[j] != 0) {
							numVicini++;
						...
						if (rigaSucc[j + 1] != 0) {
							numVicini++;
							if (sottomatrice[i][j] != rigaSucc[j + 1])
								conflitti++;
						}
					}
				}
				else if (j == COLONNE - 1) {	//ultimo elemento della prima riga
					if (sottomatrice[i][j - 1] != 0) {
						numVicini++;
						if (sottomatrice[i][j] != sottomatrice[i][j - 1])
							conflitti++;
		...
		for (i = 1; i < righe-1; i++) {
			for (j = 0; j < COLONNE; j++) {
				...
				else if(sottomatrice[i][j] != 0){ //agente tipo 1 o 2, controlla la generazione dei conflitti con i vicini

					//raccolgo a fattor comune alcuni controlli per ridurre le righe di codice
					if (sottomatrice[i - 1][j] != 0) {
						numVicini++;
						if (sottomatrice[i][j] != sottomatrice[i - 1][j])
							conflitti++;
				...
				if (conflitti > 0) { //si è verificato almeno un conflitto con questa cella
					percentuale = (100 * conflitti) / numVicini;
					if (percentuale > sogliaConflitti) {
						//printf("\n-+{r:%d}%d[i:%d][j:%d](giro:%d)", rank, *push(irregular, dimPuntatori, &sottomatrice[i][j], &k), i, j, numGiro);
						push(irregular, dimPuntatori, &sottomatrice[i][j], &k);
						numInsoddisfatti++;
		...
		//controlla i conflitti sull'ultima riga della sottomatrice, qualora la sottomatrice abbia almeno due righe
		if (i == righe - 1) {
			for (j = 0; j < COLONNE; j++) {
				if (sottomatrice[i][j] == 0 && numGiro == 1) {//crea lo stack di spazi vuoti al primo giro (dopo lo gestisce con gli scambi siccome gli spazi vuoti non cambiano di numero)
					pushQueue(empty, dimPuntatori, &sottomatrice[i][j], &head, &tail, &numVuoti);
					//printf("spazio vuoto a [%d][%d] head(%d) tail(%d) rank{%d} vuoti[%d]\n", i, j, head, tail, rank,  numVuoti);
				}
				else if (sottomatrice[i][j] != 0) {

					//raccolgo a fattor comune per ridurre il numero di if totali
					if (sottomatrice[i - 1][j] != 0) {
						numVicini++;
						if (sottomatrice[i][j] != sottomatrice[i - 1][j])
							conflitti++;
					...
					if (j == 0) {
						...
						if (rank != numtasks - 1) {//l'ultimo processo non legge dalla riga del successore
							if (rigaSucc[j] != 0) {
								numVicini++;
								if (sottomatrice[i][j] != rigaSucc[j])
									conflitti++;
					...
					else { //elementi centrali dell'ultima riga
						if (sottomatrice[i][j + 1] != 0) {
							numVicini++;
							if (sottomatrice[i][j] != sottomatrice[i][j + 1])
								conflitti++;
